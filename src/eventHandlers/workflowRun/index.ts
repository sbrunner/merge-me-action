...
interface PullRequestInformationWorkflowRun
...
): Promise<PullRequestInformationWorkflowRun | undefined> => {
...
const tryMerge = async (
  ...
  maximumRetries: number,
  {
    ...
    pullRequestState,
    reviewEdges,
  }: PullRequestInformationWorkflowRun,
): Promise<void> => {
...
export const workflowRunHandle = async (
  ...
  console.log(context);
  const pullRequests = context.payload.workflow_run.pull_requests as Array<{
    number: number;
  }>;
  ...
  for (const pullRequest of pullRequests) {
    if (
      ...
      logInfo(
        `Pull request created by ${(context.payload.sender?.login as string | undefined) ??
        'unknown sender'
